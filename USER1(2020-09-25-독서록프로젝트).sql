...
COMMIT;
DELETE FROM tbl_autority;

-- 한개의 table에 여러개의 데이터를 INSERT 하고자 할때 사용하는 다중 INSERT SQL 이다.
-- seq 값으로 PK 설정을 해두면 SQL이 작동을 하지 않는다.
INSERT ALL
...
-- 오라클에서 못쓰는 코드 시퀀스값이 있어서 안됨 MYSQL에서는 사용 가능
-- SEQ 값을 시퀀스의 NEXTVAL 값으로 설정하는 TABLE의 경우 다중 INSERT가 오류가 발생한다.
INSERT ALL 
...
-- 오라클에서 SEQ PK 칼럼을 가진 table dp 다중 insert문을 수행하기 위해서 
...
UPDATE tbl_member SET enabled = 1 WHERE m_userid = 'bb1';
commit;
